 ...
 */
#ifndef CIRCULAR_QUEUE_2_0_H
#define CIRCULAR_QUEUE_2_0_H
...
class ModeCircularQueue {
public:
    ModeCircularQueue() = delete;
    explicit ModeCircularQueue(const std::initializer_list<DriveMode>&& ill) {
        ...
        _DInfo() << "[Drive_Mode_2_0] init iter: " << static_cast<int>(std::get<kIndexDriveModeValue>(*mCurrentModeIter));
    }
    ModeCircularQueue(const ModeCircularQueue&) = delete;
    ModeCircularQueue(ModeCircularQueue&&) = delete;
    ModeCircularQueue& operator=(const ModeCircularQueue&) = delete;
    ModeCircularQueue& operator=(ModeCircularQueue&&) = delete;
    ~ModeCircularQueue() = default;
    ...
    void setCurrentMode(const DriveMode& driveMode) {
        if (mDriveModeList.empty()) {
            _DError() << "[Drive_Mode_2_0] setCurrentMode() - Drive mode list is empty";
            return;
        }
        ...
        if (iter == mDriveModeList.end()) {
            _DError() << "[Drive_Mode_2_0] setCurrentMode() - Drive mode not found: " << static_cast<int>(driveMode);
            mCurrentModeIter = mDriveModeList.begin() + 1;
            return;
        }
        ...
        _DInfo() << "[Drive_Mode_2_0] setCurrentMode() - Mode set to: "
    ...
    std::optional<DriveMode> getNextEnabledMode() {
        ...
        for (size_t i = 0; i < mDriveModeList.size(); ++i) {
            _DInfo() << "[Drive_Mode_2_0] getNextEnabledMode() iter: " << static_cast<int>(std::get<kIndexDriveModeValue>(*iter));
            ++iter;
            if (iter == mDriveModeList.end()) {
    ...
    std::optional<DriveMode> getPrevEnabledMode() {
        ...
        auto iter = mCurrentModeIter;
        for (size_t i = 0; i < mDriveModeList.size(); ++i) {
            _DInfo() << "[Drive_Mode_2_0] getPrevEnabledMode() iter: " << static_cast<int>(std::get<kIndexDriveModeValue>(*iter));
    ...
    std::optional<DriveMode> getFirstEnabledMode() {
        ...
        while (iter != mDriveModeList.end()) {
            ...
            _DInfo() << "[Drive_Mode_2_0] getFirstEnabledMode() iter: "
                     << static_cast<int>(std::get<kIndexDriveModeValue>(*iter));
            ++iter;
        }
        return std::nullopt;
    }
