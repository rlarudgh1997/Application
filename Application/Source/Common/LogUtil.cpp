...
void logHandlerForHarman(QtMsgType type, const QMessageLogContext & logContext, const QString &msg)
{
    ...
    switch ( type )
    {
    case QtDebugMsg:        localTraceServerScope.sendFormattedMessage("[%s]%s %s", logContext.category, logContext.function, msg.toLocal8Bit().data());    break;
    case QtWarningMsg:      localTraceServerScope.sendFormattedWarning("[%s]%s %s", logContext.category, logContext.function, msg.toLocal8Bit().data());    break;
    case QtCriticalMsg:     localTraceServerScope.sendFormattedError("[%s]%s %s", logContext.category, logContext.function, msg.toLocal8Bit().data());      break;
    default:                localTraceServerScope.sendFormattedMessage("[%s]%s %s", logContext.category, logContext.function, msg.toLocal8Bit().data());
    }
#else
